#!/bin/sh
# Modified version of the puppet install script
# Original repo: https://github.com/petems/puppet-install-shell
...
install_file() {
  case "$1" in
    ...
    "deb")
      ...
      apt-get update -y
      if test "$version" = 'latest'; then
        apt-get -qq install -y puppet-common puppet
      else
        case "$version" in
          [^2.7.]*)
            info "2.7.* Puppet deb package tied to Facter < 2.0.0, specifying Facter 1.7.4"
            apt-get -qq install -y puppet-common=$version-1puppetlabs1 puppet=$version-1puppetlabs1 facter=1.7.4-1puppetlabs1 --force-yes
            ;;
          *)
            apt-get -qq install -y puppet-common=$version-1puppetlabs1 puppet=$version-1puppetlabs1 --force-yes
            ;;
        esac
      fi
      ;;
...
# Delete the templatedir. See https://github.com/comperiosearch/vagrant-elk-box/issues/19
